...
const actions = {
  gameInitializeGameBoard(context, { size, id }) {
  ...
  async gameCheckGameWinner(context) {
    ...
    let transposed = context.state.gameboard[0].map((col, i) =>
      context.state.gameboard.map((row) => row[i])
    );
    ...
    await context.dispatch(
      "gameCheckHorVerLineWinner",
      transposedFormattedString
    );
